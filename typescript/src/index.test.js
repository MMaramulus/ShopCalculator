...
describe("updateSelectedServices.select", function () {
    ...
    test("should not select the same service twice", function () {
        var result = _1.updateSelectedServices(["Photography"], { type: "Select", service: "Photography" });
        expect(result).toEqual(["Photography"]);
    });
    test("should not select related service when main service is not selected", function () {
        var result = _1.updateSelectedServices(["WeddingSession"], { type: "Select", service: "BlurayPackage" });
        expect(result).toEqual(["WeddingSession"]);
    });
    test("should select related service when main service is selected", function () {
        var result = _1.updateSelectedServices(["WeddingSession", "VideoRecording"], {
            type: "Select",
    ...
    test("should select related service when one of main services is selected", function () {
        var result = _1.updateSelectedServices(["WeddingSession", "Photography"], {
            type: "Select",
            service: "TwoDayEvent"
        });
    ...
    // added tests
    test("should not allow to select twice", function () {
        var result = _1.updateSelectedServices(["WeddingSession", "Photography"], { type: "Select", service: "Photography" });
...
describe("updateSelectedServices.deselect", function () {
    ...
    test("should deselect related when last main service deselected", function () {
        var result = _1.updateSelectedServices(["WeddingSession", "Photography", "TwoDayEvent"], {
            type: "Deselect",
            service: "Photography"
        });
        expect(result).toEqual(["WeddingSession"]);
    ...
    test("should not deselect related when at least one main service stays selected", function () {
        var result = _1.updateSelectedServices(["WeddingSession", "Photography", "VideoRecording", "TwoDayEvent"], {
            type: "Deselect",
            service: "Photography"
        });
...
describe.each([2020, 2021, 2022])("calculatePrice.zero (%i)", function (year) {
    test("should be zero with no services selected", function () {
        var result = _1.calculatePrice([], year);
...
describe.each([
    ...
    test("no discount applied", function () {
        var result = _1.calculatePrice([service], year);
...
describe.each([
    ...
    test("price matches requirements", function () {
        var withoutSession = _1.calculatePrice(["Photography"], year);
        var withSession = _1.calculatePrice(["Photography", "WeddingSession"], year);
    ...
    test("discount applied", function () {
        var withoutSession = _1.calculatePrice(["Photography"], year);
        var onlySession = _1.calculatePrice(["WeddingSession"], year);
        var withSession = _1.calculatePrice(["Photography", "WeddingSession"], year);
...
describe.each([
    ...
    test("price matches requirements", function () {
        var withoutSession = _1.calculatePrice(["VideoRecording"], year);
        var withSession = _1.calculatePrice(["VideoRecording", "WeddingSession"], year);
        var priceChangeWithSession = withSession.finalPrice - withoutSession.finalPrice;
        expect(priceChangeWithSession).toEqual(increase);
    });
    test("discount applied", function () {
...
describe.each([
    ...
    test("price matches requirements", function () {
        var withoutPhotography = _1.calculatePrice(["VideoRecording"], year);
        var withPhotography = _1.calculatePrice(["VideoRecording", "Photography"], year);
    ...
    test("discount applied", function () {
        var withoutPhotography = _1.calculatePrice(["VideoRecording"], year);
        var onlyPhotography = _1.calculatePrice(["Photography"], year);
        var withPhotography = _1.calculatePrice(["VideoRecording", "Photography"], year);
...
describe.each([
    ...
    test("price matches requirements", function () {
        var withoutSession = _1.calculatePrice(["VideoRecording", "Photography"], year);
        var withSession = _1.calculatePrice(["VideoRecording", "Photography", "WeddingSession"], year);
        var priceChangeWithSession = withSession.finalPrice - withoutSession.finalPrice;
        expect(withSession.basePrice).toBeGreaterThan(0);
        expect(withSession.finalPrice).toBeGreaterThan(0);
        expect(priceChangeWithSession).toEqual(increase);
